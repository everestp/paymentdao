...
import { Search, Bell, ChevronDown, LogOut, Settings, UserCircle, Zap, CheckCheck, Menu } from 'lucide-react';
import { useApp } from '@/store/AppContext';
import { PixelAvatar, NetworkBadge } from '@/components/retro';
      ...
      <div className="flex items-center gap-3 px-4 lg:px-6 h-16">
        ...
        <div ref={userRef} className="relative">
          <button onClick={() => setUserMenuOpen(!userMenuOpen)} className="flex items-center gap-2 p-1 pr-2 border border-bdlight rounded-lg hover:border-bdbright transition-colors">
            <PixelAvatar name={user.name} color={user.avatarColor} size="sm" />
            <div className="hidden sm:block text-left">
              <div className="text-sm text-txprim font-medium">{user.name}</div>
              <div className="text-xs text-txdim font-mono">${user.personalBalance.toFixed(2)}</div>
            </div>
          ...
          {userMenuOpen && (
            <div className="absolute right-0 top-full mt-2 w-56 card bg-bgpanel z-50 p-1">
              <div className="p-3 border-b border-bdlight">
                <div className="text-sm text-txprim font-medium">{user.name}</div>
                <div className="text-xs text-txdim">{user.email}</div>
              </div>
